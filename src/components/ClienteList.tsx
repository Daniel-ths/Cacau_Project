...
export function ClienteList({ clientes, onAdd, onEdit, onDelete }: ClienteListProps) {
  ...
  const handleSave = (clienteData: Omit<Cliente, 'id' | 'dataCadastro'> & { id?: string }) => {
    if (clienteData.id) {
      // preserva dataCadastro do cliente existente antes de chamar onEdit
      const existing = clientes.find(c => c.id === clienteData.id);
      if (!existing) {
        console.error('Cliente para edição não encontrado:', clienteData.id);
        return;
      }
      const updated: Cliente = { ...existing, ...clienteData };
      onEdit(updated);
    } else {
      const { id, ...dataWithoutId } = clienteData;
      onAdd(dataWithoutId);
    }
    handleCloseForm();
  };
